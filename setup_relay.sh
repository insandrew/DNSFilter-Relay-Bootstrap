...
# ==============================================================================
# AUTOMATED DNSFILTER RELAY SETUP SCRIPT (FIXED)
...
# --- PHASE 4.5: INPUT FORMATTING (THE FIX) ---
# This section takes the raw CSV inputs and formats them into valid TOML/YAML lists
# It trims spaces, adds quotes to every item, and handles comma separation.

# 1. Format Local Domains (e.g., "corp.loc", "files.loc")
FORMATTED_LOCAL_DOMAINS=$(echo "$LOCAL_DOMAINS_INPUT" | awk -F, '{for(i=1;i<=NF;i++){gsub(/^ *| *$/,"",$i); printf "\"%s\"", $i; if(i<NF) printf ", "}}')

# 2. Format Search Domains (e.g., "curvelake.ca", "corp.local")
FORMATTED_SEARCH_DOMAINS=$(echo "$SEARCH_DOMAIN_INPUT" | awk -F, '{for(i=1;i<=NF;i++){gsub(/^ *| *$/,"",$i); printf "\"%s\"", $i; if(i<NF) printf ", "}}')

# 3. Format Local DNS IPs (Appends :53 to each IP, e.g., "192.168.1.7:53", "192.168.1.8:53")
...
# We use the formatted variables here inside the brackets [ ]
...
# Note: Explicitly using relay.conf in ExecStart to avoid fallback errors
...
# Write new Netplan
# using flow style [ list ] for search domains which accepts the quoted format we generated
